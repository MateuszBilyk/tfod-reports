...
sapply(c(10^3,10^4,10^5),function(n){
  d<-sapply(1:10^3,function(a) {
    Y<-rnorm(1, (1-0.1)*sqrt(2*log(n)), 0)
...
```{r}
apply(expand.grid(c(0.05,0.2),c(500,5000,50000)),1,function(a){
  n<-a[2]
  epsi<-a[1]
  crit_value <- sapply(1:10^5,function(i){
    Y<-rnorm(1,(1+epsi)*sqrt(2*log(n)), 0)
    Y<-append(Y,rnorm(n-1,0,1))
    return(log(ex4L(Y)))
  })  
  sort(sapply(crit_value,sum))
  crit_value<-crit_value[n/100*5]
  print(crit_value)
})
```
